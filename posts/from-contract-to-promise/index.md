...
There's a comparison hiding underneath those numbers that's worth making explicit, because it's easy to read "60-70%" as a shortfall against a contract that itself hits 100%, and that contract never did. A contract is deterministic on paper: MUST, SHOULD, MAY, no ambiguity in the text. It is not deterministic in the room where it gets enforced. A reviewer under a Friday ship date grants an exception. A clause nobody on the review team fully understands gets waved through because backlog triage doesn't leave time to chase it down. The alpine example from Example 4 is the same failure in miniature: a document that told developers to do the exact thing the platform team had actually banned, for as long as the rule lived in prose, because nobody cross-checked the example against operational reality. None of that is a hypothetical failure mode, it's what "100% compliance" costs in practice: either the review capacity to catch every deviation (which doesn't exist) or a quietly lower real number absorbed as normal.

So the honest comparison isn't Promise's 60-70% against the contract's 100%. It's Promise's 60-70%, produced in minutes and enforced by a gate with no negotiation surface, against the contract's own real-world number, produced over review cycles measured in days and enforced by whoever happened to be free that week, under whatever pressure that week happened to carry. Rule two under Security Considerations below (CRITICAL checks are never overridable from inside a skill invocation) is the part a human review process structurally can't match: a person can always be talked into an exception, a gate that hard-fails on `E1` cannot. The promise doesn't have to reach the contract's stated ideal to win that comparison. It only has to beat what the contract actually delivered once product velocity, team knowhow, and backlog review accuracy got a vote, and in most organizations that number was never close to 100% either.
